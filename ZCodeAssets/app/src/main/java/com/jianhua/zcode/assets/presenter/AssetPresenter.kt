...
class AssetPresenter @Inject constructor():BasePresenter<AssetView>(){
    ...
    fun getAssetList( assetsPorjectRequest: AssetsPorjectRequest,action: RefreshAction,listener: ProjectAssetsFragment.RequestListener){
        assetServiceImpl.getAssetList(assetsPorjectRequest)
                .execute(object:BaseSucriber<ArrayList<AssetsBean>>(baseView,AssetPresenter::class.java.simpleName){
                     ...
                     * 成功的结果返回
                     */
                    override fun onNext(t: ArrayList<AssetsBean>) {
                        super.onNext(t)
                        assertMethod(baseView,{
                            (baseView as AssetView).onGetProjectAssetList(t,action,listener)
                        })
                    }
        },lifecylerProvider)
    }
    ...
    fun getZcodeRecoderList(zCodeRecorderListRequest: ZCodeRecorderListRequest,action:RefreshAction,listener: ZCodeRecordListFragment.RequestListener){
        assetServiceImpl.getZcodeRecoderList(zCodeRecorderListRequest)
                .execute(object:BaseSucriber<ArrayList<ZCoderRecorder>>(baseView,AssetPresenter::class.java.simpleName){
                     ...
                     * 成功的结果返回
                     */
                    override fun onNext(t: ArrayList<ZCoderRecorder>) {
                        super.onNext(t)
                        assertMethod(baseView,{
                            (baseView as AssetView).onGetZCodeRecoderList(t,action,listener)
                        })
                    }
                },lifecylerProvider)
    }
