 ...
 *  @date 2018/12/1515:47
 */
class ZCodeRecordListFragment : BaseMvpRecylerviewFragment<AssetPresenter,ZCoderRecorder>(),AssetView {
...
//        getOriginAdapter().notifyDataSetChanged()
     ...
     * 创建适配器
     */
    override fun createAdapter(): RecyclerListAdapter<ZCoderRecorder, *> {
        return object : RecyclerListAdapter<ZCoderRecorder, RecyclerListAdapter.ViewHolder<ZCoderRecorder>>() {
            init {
                addViewType(ZCoderRecorder::class.java, object : RecyclerListAdapter.ViewHolderFactory<RecyclerListAdapter.ViewHolder<ZCoderRecorder>> {
                    override fun onCreateViewHolder(parent: ViewGroup): RecyclerListAdapter.ViewHolder<ZCoderRecorder> {
     ...
     * 获取资产列表 的网络请求 回调
     */
    override fun onGetZCodeRecoderList(projectAssets: ArrayList<ZCoderRecorder>, refreshAction: RefreshAction, listener: RequestListener) {
        //监听器的回调
     ...
     * 请求的监听器
     */
    open interface RequestListener {
        /**
         * 成功的回调
         */
        fun onSuccess(openProjectModels: List<ZCoderRecorder>, refreshAction: RefreshAction)
